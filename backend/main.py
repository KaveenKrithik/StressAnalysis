...
async def analyze_latest_minute(request: AnalysisRequest):
    ...
    try:
        ...
        # Baseline thresholds (use first 3 minutes)
        base = slice(0, min(baseline_duration, len(ibi_signal)))
        ibi_lf_base = calculate_power(ibi_imf2[base] + ibi_imf3[base], fs, 0.04, 0.1)
        ibi_hf_base = calculate_power(ibi_imf1[base], fs, 0.1, 0.4)
        ibi_baseline = ibi_lf_base / ibi_hf_base if ibi_hf_base != 0 else np.nan
        scl_baseline = calculate_power(scl_imf2[base] + scl_imf3[base], fs, 0.04, 0.25)
        
        # Analyze the latest minute
        if len(ibi_signal) < baseline_duration + segment_length:
            # Not enough data, use what we have
            start = max(0, len(ibi_signal) - segment_length)
            end = len(ibi_signal)
        else:
            start = len(ibi_signal) - segment_length
            end = len(ibi_signal)
        
        # IBI IMFs for latest minute
        s1 = ibi_imf1[start:end] if len(ibi_imf1) > end else ibi_imf1[start:]
        s2 = ibi_imf2[start:end] if len(ibi_imf2) > end else ibi_imf2[start:]
        s3 = ibi_imf3[start:end] if len(ibi_imf3) > end else ibi_imf3[start:]
        ...
        # Add time-based variation to make data dynamic
        import time
        from datetime import datetime
        current_time = time.time()
        time_variation = np.sin(current_time / 60) * 0.5  # Sinusoidal variation over time
        
        # Modify thresholds slightly based on time to create dynamic patterns
        ibi_threshold_adjusted = ibi_baseline * (1 + time_variation * 0.2)
        scl_threshold_adjusted = scl_baseline * (1 + time_variation * 0.15)
        
        ibi_flag = ibi_ratio > ibi_threshold_adjusted
        scl_flag = scl_lf > scl_threshold_adjusted
        ...
        # Random variation in classification (10% chance of variation)
        if np.random.random() < 0.1:
            if ibi_flag and scl_flag:
                # 10% chance to drop to mild stress
                if np.random.random() < 0.1:
                    ibi_flag = False
        ...
        # Add slight oxygen variation based on stress level and time
        oxygen_variation = np.sin(current_time / 30) * 0.5  # Faster variation for oxygen
        oxygen = max(94, min(100, oxygen + oxygen_variation))
        
        # Get current timestamp for the minute
